...
WINDOW* createWindowMenu(int starty, int startx, MENU* my_menu);
void destroyWindowMenu(WINDOW*, MENU*);
...
void CMemAccuUI::run()
{
	...
	while (1)
	{
        ...
        else if (ch == 'a' or ch == 'A')
        {
            ...
            mywin = createWindowMenu(y,x+1,my_menu);
        }
        else
...
void destroyWindowMenu(WINDOW* win, MENU* menu)
{
    if (win != NULL)
    {
        unpost_menu(menu);
